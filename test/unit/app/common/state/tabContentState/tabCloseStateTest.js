...
describe('tabCloseState unit tests', function () {
  let tabCloseState
  ...
  before(function () {
    ...
    tabCloseState = require('../../../../../../app/common/state/tabContentState/tabCloseState')
  })
  ...
  describe('hasFixedCloseIcon', function () {
    it('returns false if frame is null/undefined', function * () {
      assert.equal(tabCloseState.hasFixedCloseIcon(), false)
    })
    ...
    it('returns true if tab is active and is intersected at 75% size', function * () {
      ...
      const result = tabCloseState.hasFixedCloseIcon(state, frameKey)
  ...
  describe('hasRelativeCloseIcon', function () {
    it('returns false if frame is null/undefined', function * () {
      assert.equal(tabCloseState.hasRelativeCloseIcon(), false)
    })

    it('returns true if tab index is being hovered', function * () {
      const state = defaultState
        .setIn(['ui', 'tabs', 'hoverTabIndex'], index)
      const result = tabCloseState.hasRelativeCloseIcon(state, frameKey)
      assert.equal(result, true)
    })
  ...
  describe('showCloseTabIcon', function () {
    it('returns false if frame is null/undefined', function * () {
      assert.equal(tabCloseState.showCloseTabIcon(), false)
    })
    ...
    it('returns false if tab is being intersected at 15% size', function * () {
      const state = defaultState
        .mergeIn(['ui', 'tabs'], {
          hoverTabIndex: index,
          intersectionRatio: intersection.at15
        })
      const result = tabCloseState.showCloseTabIcon(state, frameKey)
      assert.equal(result, false)
    })
